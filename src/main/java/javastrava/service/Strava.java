...
/**
 * <p>
 * Convenience class for simplicity of access to the services. Provides a facade which delegates all javastrava.service methods to elsewhere
 * </p>
 *
 * @author Dan Shannon
 *
...
public class Strava implements ActivityService, AthleteService, ChallengeService, ClubService, ClubGroupEventService, GearService, RouteService, RunningRaceService, SegmentEffortService,
	 ...
	 *
	 */
	public Strava(final API api) {
		this.api = api;
		addServices();
	}

	private void addServices() {
		this.activityService = new ActivityServiceImpl(api);
		this.athleteService = new AthleteServiceImpl(api);
		this.challengeService = new ChallengeServiceImpl(api);
		this.clubService = new ClubServiceImpl(api);
		this.clubGroupEventService = new ClubGroupEventServiceImpl(api);
		this.gearService =  new GearServiceImpl(api);
		this.routeService = new RouteServiceImpl(api);
		this.runningRaceService = new RunningRaceServiceImpl(api);
		this.segmentEffortService = new SegmentEffortServiceImpl(api);
		this.segmentService = new SegmentServiceImpl(api);
		this.streamService = new StreamServiceImpl(api);
		this.tokenService = new TokenServiceImpl(api);
		this.uploadService = new UploadServiceImpl(api);
		this.webhookService = new WebhookServiceImpl(api);
	}
	 ...
	 */
	public Strava(final API api, final AuthorisationScope... scopes) {
		this.api = api;
		addServices();
	}
	...
	public boolean hasAuthorisationScopes(final AuthorisationScope... scopes) {
		...
		for (final AuthorisationScope scope : scopes) {
			if (!this.api.getToken().getScopes().contains(scope)) {
				return false;
			}
		}
	...
	public boolean hasExactAuthorisationScopes(final AuthorisationScope... scopes) {
		...
		for (final AuthorisationScope scope : this.api.getToken().getScopes()) {
