...
public class ListSegmentEffortsTest extends PagingListMethodTest<StravaSegmentEffort, Integer> {
	...
	public void testListSegmentEfforts_filterByAll() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			final List<StravaSegmentEffort> efforts = TestUtils.strava().listSegmentEfforts(SegmentDataUtils.SEGMENT_VALID_ID, AthleteDataUtils.ATHLETE_AUTHENTICATED_ID, startDate, endDate,
					new Paging(1, 1));
	...
	public void testListSegmentEfforts_filterByDateRange() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			final List<StravaSegmentEffort> efforts = TestUtils.strava().listSegmentEfforts(SegmentDataUtils.SEGMENT_VALID_ID, null, startDate, endDate);
			assertNotNull(efforts);
	...
	public void testListSegmentEfforts_filterByEndDate() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			final List<StravaSegmentEffort> efforts = TestUtils.strava().listSegmentEfforts(SegmentDataUtils.SEGMENT_VALID_ID, null, null, endDate);
			assertNotNull(efforts);
			assertFalse(0 == efforts.size());
			for (final StravaSegmentEffort effort : efforts) {
				assertNotNull(effort.getStartDateLocal());
	...
	public void testListSegmentEfforts_filterByInvalidAthlete() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final List<StravaSegmentEffort> efforts = TestUtils.strava().listSegmentEfforts(SegmentDataUtils.SEGMENT_VALID_ID, AthleteDataUtils.ATHLETE_INVALID_ID, null, null);
			assertNull(efforts);
		});
	}
	...
	public void testListSegmentEfforts_filterByStartDate() throws Exception {
		RateLimitedTestRunner.run(() -> {
			...
			final List<StravaSegmentEffort> efforts = TestUtils.strava().listSegmentEfforts(SegmentDataUtils.SEGMENT_VALID_ID, null, startDate, null);
			assertNotNull(efforts);
			assertFalse(0 == efforts.size());
			for (final StravaSegmentEffort effort : efforts) {
				assertNotNull(effort.getStartDateLocal());
	...
	public void testListSegmentEfforts_filterByValidAthlete() throws Exception {
		RateLimitedTestRunner.run(() -> {
			final List<StravaSegmentEffort> efforts = TestUtils.strava().listSegmentEfforts(SegmentDataUtils.SEGMENT_VALID_ID, AthleteDataUtils.ATHLETE_AUTHENTICATED_ID, null, null);
			assertNotNull(efforts);
			assertFalse(0 == efforts.size());
			for (final StravaSegmentEffort effort : efforts) {
				validate(effort);
